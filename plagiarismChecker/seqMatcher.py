...
def plagerised_ratio(filename1, filename2,ext1,ext2):
    print(type(filename1))
    tokens1 = tokenize(filename1,ext1) #(elements of cleaned up code, their position in original code, position in cleaned up code)
    file1 = toText(tokens1)  #cleaned up code - greatly increases effectiveness of plagiarism checker
    tokens2 = tokenize(filename2,ext2)
    ...
    for i in blocks:
        ...
        if not flag == 0 and (end - start) > 100:  #printing significant blocks of plagiarized content
            ...
            # filename1.seek(start, 0)
            print(type(filename1))
            # print(filename1.read(end - start))
            return(similarity_ratio,filename1[start:end+1])
